...
//     return(
//         <div  className={`flex items-center justify-between px-6 md:px-16 lg:ppx-24 
...
//             <div className="hidden lg:flex items-center text-center text-sm gap-2 border
...
const Navbar = ({}) => {
  ...
  // Close mobile menu whenever the route changes (prevents overlay staying open)
  useEffect(() => {
    setOpen(false);
  }, [location.pathname]);
  ...
  return (
    ...
    >
      ...
      >
        {menuLinks.map((link, index) => (
          <Link key={index} to={link.path} onClick={handleNavClick}>
            {link.name}
          </Link>
        ))}

        <div
          className="hidden lg:flex items-center text-center text-sm gap-2 border
            border-borderColor px-3 rounded-full max-w-56"
        >
          <input
            type="text"
