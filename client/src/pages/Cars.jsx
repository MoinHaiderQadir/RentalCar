...
const Cars = ()=>{
    ...
    // getting search paramas from url 
    const [searchParams] =  useSearchParams()
    const pickupLocation = searchParams.get('pickupLocation')
    const pickupDate = searchParams.get('pickupDate')
    const returnDate = searchParams.get('reurnDate')

    const {cars, axios} = useAppContext()

    const isSearchData = pickupLocation && pickupDate && returnDate
    const [filteredCars, sertFilteredCars] = useState([])

    const applyFilter = async ()=>{
        if(input === ''){
            sertFilteredCars(cars)
            return null
        }
    ...
    const searchCarAvailablity = async()=>{
        const {data}= await axios.post('/api/bookings/check-availability',
            ...
            if(data.success){
                sertFilteredCars(data.availableCars)
    ...
    return (
        <div>
            ...
            <div className="px-6 md:px-16 lg:px-24 xl:px-32 mt-10" >
                <p className="">Showing {filteredCars.length} Cars</p>

                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 mt-4
                xl:px-20max-w-7xl mx-auto">
